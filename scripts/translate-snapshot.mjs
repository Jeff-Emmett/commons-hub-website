...
const isTranslatableVal = (v) =>
  ...
  /[A-Za-z]/.test(v) && !SKIP_VAL.test(v.trim());
...
function collect(node, key, out) {
  ...
  if (isTranslatableKey(key) && isTranslatableVal(node)) out.add(node);
}
...
function collectOrdered(node, key, out) {
  ...
  if (isTranslatableKey(key) && isTranslatableVal(node)) out.push(node);
}
...
function rewrite(node, key, map) {
  ...
  if (isTranslatableKey(key) && isTranslatableVal(node) && map.has(node)) return map.get(node);
  return node;
}
